...
`/api/health` reports configuration readiness, not just liveness: it returns `status: 'degraded'` and `config: { groqKey, rateLimit }` so a deploy that is missing `GROQ_API_KEY` is detectable (previously it returned `ok` while `/api/postprocess` 500'd on every call). Point probes at `config.groqKey`.
...
### Security headers are split across two places (deliberate)

This trips people up, so it is written down: **Cloudflare applies `public/_headers` only to static-asset responses** (the SPA shell, `/assets/*`, `manifest.json`, icons…). Responses generated by Worker code — every `/api/*` — are **not** covered, even when the URL matches a rule (this is an explicit warning in Cloudflare's docs). So:

- `public/_headers` — asset responses: `X-Content-Type-Options`, `Referrer-Policy`, `X-Frame-Options`, `Permissions-Policy`, HSTS, plus a **report-only** CSP, and `immutable` caching for the fingerprinted `/assets/*`.
- [worker/middleware/security-headers.ts](worker/middleware/security-headers.ts) — API responses: `nosniff`, `no-referrer`, `Cache-Control: no-store`.

Two things to know before touching the CSP:

1. **The CSP is report-only on purpose.** It has not been browser-verified, and the two things it would most plausibly break are YouTube playback and BYOK. Flip it to `Content-Security-Policy` only after smoke-testing both in a browser.
2. **`connect-src` is coupled to the provider catalog.** BYOK calls providers straight from the browser, so every origin in [src/lib/ai/catalog.ts](src/lib/ai/catalog.ts) must be listed there. Adding a provider without updating the CSP shows up as "my key doesn't work" with the only clue in the browser console — so [src/lib/ai/__tests__/csp-catalog-sync.test.ts](src/lib/ai/__tests__/csp-catalog-sync.test.ts) fails the build instead.
